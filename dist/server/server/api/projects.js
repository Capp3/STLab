...
router.get('/:id', async (req, res, next) => {
    try {
        ...
        for (const r of violationRows) {
            if (r.severity === 'error')
                vcounts.error = Number(r.c);
            if (r.severity === 'warning')
                vcounts.warning = Number(r.c);
        }
        res.json({
            data: {
                ...project,
...
router.patch('/:id', async (req, res, next) => {
    try {
        const { name, description } = req.body;
        const existing = await db.query.projects.findFirst({ where: eq(projects.id, req.params['id'] ?? '') });
...
router.delete('/:id', async (req, res, next) => {
    try {
        ...
        if (!existing || existing.deletedAt)
            throw createError('Project not found', 404, 'NOT_FOUND');
        await db.update(projects).set({ deletedAt: new Date() }).where(eq(projects.id, req.params['id'] ?? ''));
