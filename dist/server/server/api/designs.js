...
router.put('/', async (req, res, next) => {
    try {
        ...
        if (!graph?.nodes || !graph?.links)
            throw createError('Invalid design graph', 400, 'VALIDATION_ERROR');
        ...
        await db.transaction(async (tx) => {
            ...
            if (graph.nodes.length > 0) {
                await tx.insert(nodes).values(graph.nodes.map((n) => ({
                    id: n.id || uuidv4(),
                    projectId,
                    nodeType: n.nodeType,
                    label: n.label,
            ...
            if (graph.links.length > 0) {
                await tx.insert(links).values(graph.links.map((l) => ({
                    id: l.id || uuidv4(),
                    projectId,
                    sourceNodeId: l.sourceNodeId,
                    targetNodeId: l.targetNodeId,
            ...
            if (graph.flows && graph.flows.length > 0) {
                await tx.insert(flows).values(graph.flows.map((f) => ({
                    id: f.id || uuidv4(),
                    linkId: f.linkId,
                    projectId,
                    essenceType: f.essenceType,
                    bandwidthMbps: f.bandwidthMbps,
                    st2022_7Protected: f.properties.st2022_7Protected ?? false,
                    senderType: f.properties.senderType ?? null,
                    properties: f.properties,
                })));
            }
