...
router.post('/', async (req, res, next) => {
    try {
        ...
        if (!['html', 'pdf'].includes(format))
            throw createError('format must be html or pdf', 400, 'VALIDATION_ERROR');
        ...
        if (format === 'pdf') {
            try {
                ...
                console.warn('[reports] PDF generation failed, returning HTML:', pdfErr.message);
...
router.get('/:reportId/download', async (req, res, next) => {
    try {
        ...
        if (!report || !report.artifactPath)
            throw createError('Report not found or not ready', 404, 'NOT_FOUND');
        if (!existsSync(report.artifactPath))
            throw createError('Report file not found', 404, 'NOT_FOUND');
