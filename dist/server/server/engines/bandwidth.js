 ...
 */
export function computeOverheadBytes(vlanTagged) {
 ...
 */
export function computeAncBandwidth() {
    return {
        mbps: 0.075, // 75 kbps midpoint of 50–100 kbps range
        confidence: 'low',
        assumption: 'ASSUMPTION: ANC bandwidth is non-normative. Typical 50–100 kbps (observational). Verify per deployment.',
    };
}
...
function effectiveFlowBandwidth(flow) {
    ...
    const vlanTagged = props['vlanTagged'] ?? false;
 ...
 */
export async function computeLinkMetrics(linkId) {
 ...
 */
export async function computeSwitchMetrics(nodeId, projectId) {
    const node = await db.query.nodes.findFirst({ where: and(eq(nodes.id, nodeId), eq(nodes.projectId, projectId)) });
    if (!node || !['dedicated_switch', 'shared_switch'].includes(node.nodeType))
        return null;
    const props = node.properties;
    const backplaneCapacityGbps = props['backplaneCapacityGbps'] ?? 0;
    const existingLoadGbps = props['existingNonSt2110LoadGbps'] ?? 0;
 ...
 */
export async function runBandwidthEngine(projectId) {
    ...
    for (const link of projectLinks) {
        ...
        if (ratio >= UTILIZATION_ERROR_THRESHOLD) {
            newViolations.push({
                id: uuidv4(), projectId,
                entityId: link.id, entityType: 'link',
                violationType: 'BANDWIDTH_EXCEEDED',
                severity: 'error',
                message: `Link "${link.label ?? link.id}" is over capacity: ${metrics.utilizationPct.toFixed(1)}%`,
                detail: { utilizationPct: metrics.utilizationPct, usedMbps: metrics.usedMbps, capacityMbps: metrics.capacityMbps },
            });
        }
        ...
        if (metrics.wideSenderCount > 0) {
            newViolations.push({
                id: uuidv4(), projectId,
                entityId: link.id, entityType: 'link',
                violationType: 'WIDE_SENDER_BUFFER_RISK',
                severity: 'warning',
    ...
    for (const node of projectNodes.filter((n) => ['dedicated_switch', 'shared_switch'].includes(n.nodeType))) {
        ...
        newMetrics.push({
            id: uuidv4(), projectId,
            entityId: node.id, entityType: 'node',
        ...
        if (metrics.backplaneCapacityGbps > 0) {
            if (metrics.backplaneUtilizationPct >= 100) {
                newViolations.push({
                    id: uuidv4(), projectId,
                    entityId: node.id, entityType: 'node',
                    violationType: 'BACKPLANE_EXCEEDED',
                    severity: 'error',
    ...
    if (newMetrics.length > 0)
        await db.insert(derivedMetrics).values(newMetrics);
