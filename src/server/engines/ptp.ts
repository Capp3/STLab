...
export interface PtpDomainTrace {
  domainNumber: number
...
export interface PtpEngineResult {
  domains: PtpDomainTrace[]
  ptpViolations: Array<{
    type: ViolationType
 ...
 */
function buildPtpAdjacency(ptpLinks: Array<{ id: string; sourceNodeId: string; targetNodeId: string }>): Map<string, string[]> {
  const adj = new Map<string, string[]>()
  for (const link of ptpLinks) {
    if (!adj.has(link.sourceNodeId)) adj.set(link.sourceNodeId, [])
    if (!adj.has(link.targetNodeId)) adj.set(link.targetNodeId, [])
 ...
 */
function getLinkPtpDomain(linkProps: unknown): number | undefined {
  const p = linkProps as Record<string, unknown>
 ...
 */
export async function tracePtpDomains(projectId: string): Promise<PtpEngineResult> {
  ...
  const engineViolations: PtpEngineResult['ptpViolations'] = []
  const domains: PtpDomainTrace[] = []

  // Track global reachability (all nodes reached from any GM)
  const globalReachable = new Set<string>()

  // BFS from each Grandmaster Clock
  for (const gm of grandmasters) {
    const props = gm.properties as Record<string, unknown>
    const domainNumber = (props['domainNumber'] as number | undefined) ?? 127
    ...
    while (queue.length > 0) {
      ...
      for (const neighborId of adj.get(nodeId) ?? []) {
        ...
        const traversedLink = ptpLinks.find(
          (l) =>
            (l.sourceNodeId === nodeId && l.targetNodeId === neighborId) ||
 ...
 */
export async function runPtpEngine(projectId: string): Promise<void> {
