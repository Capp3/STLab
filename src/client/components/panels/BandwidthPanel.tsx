  ...
  return (
    <div>
      <div className="stlab-panel-section">
        ...
        {bwMetrics.length === 0 && (
          <div style={{ color: '#64748b', fontSize: 12 }}>No link metrics — save design to compute</div>
        )}
        {bwMetrics.map((m) => {
          ...
          {switchMetrics.map((m) => {
            ...
            return (
              <div key={m.entityId} style={{ marginBottom: 12 }}>
                ...
                <div style={{ background: '#0f3460', height: 6, borderRadius: 3, overflow: 'hidden' }}>
                  <div
                    style={{ width: `${Math.min(util, 100)}%`, height: '100%', background: color, borderRadius: 3 }}
                  />
                </div>
                <div style={{ fontSize: 10, color: '#64748b', marginTop: 2 }}>
                  In: {(v['totalIngressMbps'] ?? 0).toFixed(0)} Mbps | Out: {(v['totalEgressMbps'] ?? 0).toFixed(0)}{' '}
                  Mbps
                </div>
              </div>
            );
